  ...
  if (punches && punches.length > 0) {
    punches.forEach(punch => {
      try {
        ...
        // **CRITICAL FIX**: Filter out future hours BEFORE calculating proportions
        ...
        console.log(`📊 SHIFT PROCESSING: Processing shift ${punchStartHourMT}:00-${punchEndHourMT}:00, allowed hours: ${allowedHours.join(',')}, total shift hours: ${punch.hoursWorked}`);
        
        allowedHours.forEach(hourNum => {
          const hourKey = `${hourNum.toString().padStart(2, '0')}:00`;
          
          if (hourlyData[hourKey]) {
            // **FIXED CALCULATION**: Only divide by the allowed hours, not the original hours that included future time
            ...
            // Use this corrected distribution
