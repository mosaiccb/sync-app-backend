...
import { UkgReadyApiService, UkgReadyConfig, UkgReadyEmployeePayload } from '../services/UkgReadyApiService';

// PAR Brink Employee Interface (based on our successful API test)
interface ParBrinkEmployee {
    EmployeeId: string;
    FirstName: string;
    LastName: string;
    ...
    PayRate: number;
    IsActive: boolean;
    // Additional fields from PAR Brink response
    MiddleName?: string;
...
// UKG Ready Employee Interface (target format)
interface UkgReadyEmployee extends UkgReadyEmployeePayload {
    // Additional fields specific to our ETL process
}

// Transformation mappings
const JOB_CODE_MAPPING: Record<string, string> = {
    '10': 'Manager',
    '20': 'Crew Member',
    '30': 'Shift Leader',
...
const SECURITY_LEVEL_MAPPING: Record<string, string> = {
    '1': 'Manager',
    '2': 'Staff',
    '5': 'Shift Leader',
    // Add more mappings as needed
 ...
 */
export async function parBrinkToUkgETL(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    try {
 ...
 */
async function extractParBrinkEmployees(accessToken: string, locationToken: string, context: InvocationContext): Promise<ParBrinkEmployee[]> {
 ...
 */
function parseParBrinkEmployeeXML(xmlText: string, context: InvocationContext): ParBrinkEmployee[] {
    const employees: ParBrinkEmployee[] = [];
    ...
    try {
        ...
        employeeMatches.forEach(employeeXml => {
            const employee: ParBrinkEmployee = {
 ...
 */
function extractXmlValue(xml: string, elementName: string): string | undefined {
 ...
 */
function transformParBrinkToUkgReady(parBrinkEmployees: ParBrinkEmployee[], context: InvocationContext): UkgReadyEmployee[] {
    ...
    return parBrinkEmployees.map(parEmployee => {
        const ukgEmployee: UkgReadyEmployee = {
 ...
 */
function formatDateForUkg(dateString: string): string {
    try {
 ...
 */
async function dryRunPreview(ukgEmployees: UkgReadyEmployee[], context: InvocationContext): Promise<any> {
    ...
    const stats = {
        ...
        jobTitleDistribution: {} as Record<string, number>,
 ...
 */
async function loadToUkgReady(ukgEmployees: UkgReadyEmployee[], ukgConfig: UkgReadyConfig, context: InvocationContext): Promise<any> {
    context.log(`📤 Loading ${ukgEmployees.length} employees to UKG Ready...`);
    
    try {
        const ukgService = new UkgReadyApiService(ukgConfig, context);
...
export async function getParBrinkEmployeesTest(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    try {
        const requestBody = await request.text();
        const config = JSON.parse(requestBody);
...
app.http('getParBrinkEmployeesTest', {
    methods: ['POST', 'OPTIONS'],
    authLevel: 'anonymous',
...
export async function testUkgReadyConnection(request: HttpRequest, context: InvocationContext): Promise<HttpResponseInit> {
    try {
        const requestBody = await request.text();
...
app.http('testUkgReadyConnection', {
    methods: ['POST', 'OPTIONS'],
    authLevel: 'anonymous',
