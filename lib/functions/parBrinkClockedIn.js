...
exports.parBrinkClockedIn = parBrinkClockedIn;
 ...
 */
async function parBrinkClockedIn(request, context) {
    try {
        ...
        const responseData = {
            ...
            clockedInEmployees: clockedInEmployees.sort((a, b) => new Date(a.clockInTime).getTime() - new Date(b.clockInTime).getTime()),
 ...
 */
async function fetchParBrinkEmployees(accessToken, locationToken, context) {
    try {
    ...
    <soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:set="http://www.brinksoftware.com/webservices/settings/v2">
        ...
        const response = await axios_1.default.post('https://api11.brinkpos.net/Settings2.svc', soapEnvelope, {
            headers: {
 ...
 */
async function fetchCurrentTimeEntries(accessToken, locationToken, businessDate, offsetMinutes, _timezone, context) {
    try {
    ...
    <soap:Envelope xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/" xmlns:lab="http://www.brinksoftware.com/webservices/labor/v2">
        ...
        const response = await axios_1.default.post('https://api11.brinkpos.net/Labor2.svc', soapEnvelope, {
            headers: {
                'AccessToken': accessToken,
                'LocationToken': locationToken,
                'Content-Type': 'text/xml',
 ...
 */
function parseEmployeeXML(xmlData, context) {
    const employees = [];
    try {
 ...
 */
function parsePunchXML(xmlData, context) {
    const timeEntries = [];
    try {
 ...
 */
function processClockEntries(employees, timeEntries, timezone, context) {
 ...
 */
function determinePunchStatus(punchType) {
 ...
 */
function determineCurrentStatus(entries, currentTime) {
    let isWorking = false;
    let currentStatus = 'clocked-in';
    let clockInTime;
    let location = '';
...
functions_1.app.http('parBrinkClockedIn', {
    methods: ['GET', 'POST', 'OPTIONS'],
    authLevel: 'anonymous',
    route: 'par-brink/clocked-in',
    handler: parBrinkClockedIn
});
//# sourceMappingURL=parBrinkClockedIn.js.map