...
exports.getParBrinkLaborHourDetail = getParBrinkLaborHourDetail;
 ...
 */
async function enforceParBrinkRateLimit(context) {
 ...
 */
function releaseParBrinkRateLimit() {
...
async function callRealParBrinkSales(accessToken, locationToken, businessDate, context) {
    ...
    try {
        ...
        // Fall back to simulated sales data with timezone support
        return generateRealisticSalesData(businessDate, testLocationToken, timezone);
    }
    finally {
        // Release rate limiting counter
        releaseParBrinkRateLimit();
 ...
 */
function generateRealisticSalesData(businessDate, locationToken, timezone = 'America/Denver') {
 ...
 */
function getCurrentTimeInTimezone(timezone = 'America/Denver', includeTime = false) {
    const now = new Date();
 ...
 */
function getTimezoneOffsetMinutes(timezone = 'America/Denver') {
    const now = new Date();
 ...
 */
function getEnhancedLocationMapping() {
    return {
 ...
 */
async function getParBrinkLaborHourDetail(request, context) {
    try {
        ...
        const activeShifts = laborData.shifts.filter((shift) => {
        ...
        const employeesByPosition = activeShifts.reduce((acc, shift) => {
        ...
        const hourDetail = {
            ...
            employees: activeShifts.map((shift) => ({
...
async function getLocationsWithTimezones(_request, context) {
    try {
...
functions_1.app.http('getParBrinkLaborHourDetail', {
    methods: ['POST', 'OPTIONS'],
    authLevel: 'anonymous',
...
functions_1.app.http('getLocationsWithTimezones', {
    methods: ['GET', 'OPTIONS'],
    authLevel: 'anonymous',
