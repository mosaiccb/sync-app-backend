 ...
 */
async function weatherDashboard(request, context) {
    try {
...
async function handleHealthCheck(context) {
    const weatherHealth = await weatherService_1.weatherService.healthCheck(context);
    const storeHealth = await storeConfigService_1.storeConfigService.healthCheck(context);
    const overallStatus = weatherHealth.status === 'healthy' && storeHealth.cacheStatus === 'healthy'
        ? 'healthy'
        : 'degraded';
    return {
...
async function handleGetWeather(request, context) {
    ...
    if (store.dailyHours) {
        ...
        const todayHours = store.dailyHours[today];
        if (todayHours) {
            businessHours = {
    ...
    // Get weather data
    const weatherData = await weatherService_1.weatherService.getRestaurantWeather(store.address, businessHours, context);
    if (!weatherData) {
        return {
            status: 503,
            headers: {
...
async function handleListStoreWeather(request, context) {
...
functions_1.app.http('weatherDashboard', {
