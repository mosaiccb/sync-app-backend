 ...
 */
function getCurrentTimeInLocationTimezone(timezone = 'America/Denver', includeTime = false) {
    const now = new Date();
 ...
 */
function getLocationTimezoneOffsetMinutes(timezone = 'America/Denver') {
    const now = new Date();
 ...
 */
function getEnhancedLocationMappingForDashboard() {
    return {
...
async function fetchParBrinkSalesData(accessToken, locationToken, businessDate, context, locationTimezone = 'America/Denver') {
    try {
        const headers = {
 ...
 */
function convertToLocationTimezone(dateString, timezone = 'America/Denver') {
...
function processHourlyLaborData(_punches) {
    const hourlyData = {};
...
function parseOrdersFromXML(_xmlData) {
    try {
