...
async function fetchParBrinkSalesData(accessToken, locationToken, businessDate, timezone, context) {
    try {
        const headers = {
...
function getRestaurantBusinessDate() {
...
function getRestaurantBusinessDateForTimezone(timezone) {
    // For restaurants, the business day typically runs from early morning (5-6 AM) 
    // until late night/early morning (2-3 AM next day)
...
function convertToLocalTime(dateString, timezone) {
    const date = new Date(dateString);
