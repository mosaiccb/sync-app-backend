...
class TenantDatabaseService {
    ...
    constructor() {
        ...
        // SQL_AUTH_TYPE options:
        // - 'default': Standard SQL authentication with username/password
        // - 'ntlm': NTLM authentication
        // - 'azure-active-directory-password': Azure AD password authentication
        // - 'azure-active-directory-access-token': Azure AD access token
        // - 'azure-active-directory-default': Azure AD default credential
        // - 'azure-active-directory-msi-vm': Managed Identity for VM
        // - 'azure-active-directory-msi-app-service': Managed Identity for App Service
        // - 'azure-active-directory-service-principal-secret': Service Principal
        const authType = process.env.SQL_AUTH_TYPE || 'default';
        if (connectionString) {
            // Parse connection string
            const config = this.parseConnectionString(connectionString);
            // For managed identity authentication, don't include username/password in options
            const authOptions = {};
            if (authType === 'default' && config.username && config.password) {
                authOptions.userName = config.username;
            ...
            this.connectionConfig = {
                ...
                options: {
                    ...
                    encrypt: config.encrypt !== false, // Default to true if not specified
            ...
            const authOptions = {};
            if (authType === 'default') {
                authOptions.userName = process.env.SQL_USERNAME || '';
                authOptions.password = process.env.SQL_PASSWORD || '';
            }
            this.connectionConfig = {
...
//# sourceMappingURL=TenantDatabaseService-ChuckDtop.js.map