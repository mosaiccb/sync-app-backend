...
exports.testColoradoStoresCoverage = testColoradoStoresCoverage;
exports.runAllTests = runAllTests;
const storeConfigService_1 = require("../services/storeConfigService");
const databaseStoreService_1 = require("../services/databaseStoreService");
/**
 * Performance test: SQL vs Cache
 */
async function performanceTest(context) {
    const testTokens = [
    ...
    for (const token of testTokens) {
        const result = await timeFunction(() => databaseStoreService_1.databaseStoreService.getStoreByToken(token, context), 'Database Query');
    ...
    for (const token of testTokens) {
        const result = await timeFunction(() => storeConfigService_1.storeConfigService.getStoreConfig(token, context), 'Cache Lookup');
        if (result.success && result.data) {
 ...
 */
async function testEnhancedFeatures(context) {
    const testToken = "RPNrrDYtnke+OHNLfy74/A=="; // Castle Rock
    const store = await storeConfigService_1.storeConfigService.getStoreConfig(testToken, context);
    if (!store) {
 ...
 */
async function testOperatingHours(context) {
    const testToken = "RPNrrDYtnke+OHNLfy74/A=="; // Castle Rock
    const store = await storeConfigService_1.storeConfigService.getStoreConfig(testToken, context);
    if (!store) {
        context.log('  ❌ Could not retrieve test store');
    ...
    if (store.dailyHours) {
        ...
        for (const day of days) {
            const dayHours = store.dailyHours[day];
            if (dayHours) {
 ...
 */
async function timeFunction(fn, _label) {
 ...
 */
async function testColoradoStoresCoverage(context) {
    ...
    for (const storeInfo of coloradoTokens) {
        const store = await storeConfigService_1.storeConfigService.getStoreConfig(storeInfo.token, context);
        if (store) {
...
//# sourceMappingURL=enhancedStoreTests.js.map