...
IF EXISTS (SELECT * FROM sys.views WHERE name = 'ActiveTenantsWithUKG')
    DROP VIEW [dbo].[ActiveTenantsWithUKG];

IF EXISTS (SELECT * FROM sys.procedures WHERE name = 'DeleteTenant')
    DROP PROCEDURE [dbo].[DeleteTenant];

IF EXISTS (SELECT * FROM sys.procedures WHERE name = 'UpdateTenant')
    DROP PROCEDURE [dbo].[UpdateTenant];

IF EXISTS (SELECT * FROM sys.procedures WHERE name = 'CreateTenant')
    DROP PROCEDURE [dbo].[CreateTenant];

-- Drop tables in correct order (due to foreign key constraints)
IF EXISTS (SELECT * FROM sys.tables WHERE name = 'TenantAudit')
    DROP TABLE [dbo].[TenantAudit];

IF EXISTS (SELECT * FROM sys.tables WHERE name = 'UKGApiConfigurations')
    DROP TABLE [dbo].[UKGApiConfigurations];
...
-- Test the schema by querying the view
SELECT * FROM [dbo].[ActiveTenantsWithUKG];
...
UNION ALL
SELECT 
    'UKGApiConfigurations' AS TableName,
    COUNT(*) AS RecordCount
FROM [dbo].[UKGApiConfigurations]
UNION ALL
SELECT 
    'TenantAudit' AS TableName,
    COUNT(*) AS RecordCount
FROM [dbo].[TenantAudit];
